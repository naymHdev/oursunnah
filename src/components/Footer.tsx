        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-4">
            <a href="#" className="flex flex-col mb-8">
              <div className="flex items-center gap-3 mb-1.5">
                ...
                <span className="font-serif text-3xl tracking-[0.25em] uppercase text-brand-cream">Noor</span>
                <div className="w-px h-6 bg-brand-gold" />
              </div>
            ...
            <p className="text-brand-cream/50 font-sans text-sm leading-relaxed max-w-xs mb-8">
              A modern Islamic lifestyle brand rooted in craftsmanship, heritage, and mindful living. Every piece is created with intention.
            </p>

            <div className="space-y-3 text-sm mb-8">
              <div className="flex items-start gap-3 text-brand-cream/40">
                <MapPin size={13} className="text-brand-gold mt-0.5 shrink-0" />
                <span className="font-sans">Downtown Dubai, UAE — Global Flagship</span>
              </div>
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category} className="lg:col-span-2">
              <h4 className="text-label uppercase tracking-widest text-brand-gold mb-6">{category}</h4>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    ...
                    >
                      <ArrowRight size={10} className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-brand-gold" />
                      {link}
                    </a>
                  </li>
                ))}
          ...
          <div className="lg:col-span-2">
            <h4 className="text-label uppercase tracking-widest text-brand-gold mb-6">Secure Payment</h4>
            <div className="grid grid-cols-3 gap-2 mb-8">
              {['Visa', 'MC', 'AMEX', 'Apple', 'Google', 'PayPal'].map((card) => (
                <div key={card} className="h-8 border border-brand-cream/10 flex items-center justify-center">
                  <span className="text-brand-cream/30 text-[9px] font-sans tracking-wider uppercase">{card}</span>
                </div>
              ))}
            </div>
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-brand-cream/30 text-label">
                <div className="w-1.5 h-1.5 rounded-full bg-brand-gold" />
                <span className="uppercase tracking-widest">Halal Certified</span>
              </div>
              <div className="flex items-center gap-2 text-brand-cream/30 text-label">
                <div className="w-1.5 h-1.5 rounded-full bg-brand-gold" />
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            {['Privacy Policy', 'Terms of Use', 'Cookie Settings'].map((item) => (
              <a key={item} href="#" className="text-brand-cream/25 font-sans text-xs hover:text-brand-cream/60 transition-colors duration-300">
                {item}
              </a>
            ))}
          </div>
          <p className="text-brand-cream/15 font-sans text-xs">
            Crafted with intention — <span className="text-brand-gold/40">بِسْمِ اللَّهِ</span>
          </p>
        </div>
      </div>
    </footer>
